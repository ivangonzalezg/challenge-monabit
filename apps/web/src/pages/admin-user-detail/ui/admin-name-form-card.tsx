...
import { useUpdateUserName } from "../model"
import {
  Button,
  Card,
  CardContent,
...
type AdminNameFormCardProps = {
  userId: string
  currentName: string
}

export function AdminNameFormCard({
  userId,
  currentName,
}: AdminNameFormCardProps) {
  ...
  const onSubmit = async (values: NameFormValues) => {
    try {
      await updateUserName.mutateAsync(values.name)
    ...
    <Card>
      <form onSubmit={form.handleSubmit(onSubmit)} noValidate>
        <CardHeader>
          ...
          <CardDescription>
            Actualiza el nombre que se muestra en la cuenta de este usuario.
          </CardDescription>
        </CardHeader>
