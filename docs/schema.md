...
- **Extend, don't duplicate.** MonaBit adds application-level fields (`role`, `banned`, `banReason`, `banExpires`) to the Better Auth user. It does not create a competing user table.
...
### `user` (Better Auth — extended by MonaBit)

Better Auth owns this table. MonaBit adds these columns via the Drizzle schema:
